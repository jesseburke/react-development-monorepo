...
export default function App() {
    ...
    const optionsButtonCallback = useCallback(() => setShowOptsDrawer((o) => !o), []);

    const cameraChangeCB = useCallback(
        (position) => {
            if (position) setCameraData((cd) => ({ ...cd, position }));
    ...
    return (
        <FullScreenBaseComponent backgroundColor={colors.controlBar} fonts={fonts}>
            <ControlBar height={controlBarHeight} fontSize={initFontSize * percControlBar}>
                ...
                <Button fontSize={initFontSize * percButton} onClickFunc={optionsButtonCallback}>
                    <div>
            ...
            <Main height={100 - controlBarHeight} fontSize={initFontSize * percDrawer}>
                ...
                <Modal
                    ...
                    <CoordinateOptions
                        axesData={axesData}
                        gridData={gridData}
                        onAxesChange={useCallback(
                            (newData) => setAxesData((ad) => ({ ...ad, ...newData })),
                            []
                        )}
                        onGridChange={useCallback(
                            (newData) => setGridData((gd) => ({ ...gd, ...newData })),
                            []
                        )}
                    />
                </Modal>
