...
const initFuncStr = 'x+y';// "x*y*sin(x + y)/10";
...
{position: [0, 0, 100],
 ...
 near: -1,
 far: 5000,
 orthographic: { left: cameraConst*(-halfXSize),
...
export default function App() {   
    ...
    const clickCB = useCallback( (pt) => {
        ...
        if( pt.x > halfXSize - .5 || pt.x < -halfXSize + .5 ) {
            setInitialPt( null );
            return;
        }
        ...
        <FullScreenBaseComponent backgroundColor={colors.controlBar}
          ...
          <ControlBar height={controlBarHeight} fontSize={initFontSize*percControlBar} padding='.5em'>
	        ...
	        dy/dx = h(y)g(x)
              ...
              <div>
                <span css={{padding: '1em'}}>h(y) = <Input size={4}
                                    initValue={initYFuncStr}
                                    onC={yFuncInputCB}/></span>
                <span>g(x) = <Input size={4}
