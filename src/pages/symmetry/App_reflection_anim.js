...
export default function App() {
    ...
    // draw two copies of square, one will stay fixed, other will be reflected
      ...
      useEffect( () => {
          ...
          const geom = LinePathGeom( RegularNgonPts(4),
                                     .025, 1 ).rotateZ(-Math.PI/4).scale(scale, scale, scale);
	  
          const redMat = new THREE.MeshBasicMaterial({ color: 'rgb(255, 0, 0)', opacity: .5 });
          ...
          const blueMat = new THREE.MeshBasicMaterial({ color: 'rgb(0, 0, 255)', opacity: .5 });
    ...
    const textFadeDuration = .5;   
    const startStopDelay = .5;
	...
	let planeFigLabel = threeCBs.addLabel( { pos: [-3.5, -2, 0],
						 text: ' \u{2B05} plane figure',
						 style: labelStyle } );
        threeCBs.drawLabels();

        const tl =  gsap.timeline({ repeat: 0, repeatDelay: .5, paused,
				    onStart: () => {
        ...
        tl.add( gsapAnimation1({ threeCBs,
                                 startStopDelay,
                                 ref: fullScreenRef.current,
                                 mesh: squareMeshR }) );
        
        tl.add( gsapAnimation2({ threeCBs,
                                 startStopDelay,
                                 ref: fullScreenRef.current,
                                 mesh: squareMeshR,
			       }),`+=1` );
    ...
    function gsapAnimation1({ threeCBs, startStopDelay = .5, ref, mesh }) {

        if( !threeCBs ) return;

	const vec = new THREE.Vector3(1,1,0);
        const lineGeom = OriginLine({ radius: .04, vec  });
	...
	let transfLabelDiv = makeDiv({ text: 'reflection about y = x',
	...
	//tl.add( () => null, '+=.5' );
        ...
        const textAnim = gsapTextAnimation({ parentNode: ref,
	...
	tl.to(transfLabelDiv, {duration: .5, opacity: 0});
    ...
    function gsapAnimation2({ threeCBs, startStopDelay = .5, ref, mesh }) {
	...
	let transfLabelDiv = makeDiv({ text: 'reflection about 2y = x',
				       id: 'reflectionDisplay',
				       style: transfLabelStyle });
	document.body.append(transfLabelDiv);
	...
	const reflectAnim =  gsapRotate(
        ...
        const textAnim = gsapTextAnimation({ parentNode: fullScreenRef.current,
                                             style: symmLabelStyle,
                                             entrySide: 'left',
					     duration: textFadeDuration,
					     displayTime: textDisplayDuration,
