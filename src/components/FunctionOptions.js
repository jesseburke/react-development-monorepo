    ...
    return (
        <div css={{
          ...
          <div css={{
              ...
              display: 'flex',
              justifyContent: 'space-between'}}>
            <FunctionBounds bounds={{xMin:newData.xMin, xMax:newData.xMax,
                                     yMin: newData.yMin, yMax: newData.yMax}}
                            onChange={ ({xMin, xMax, yMin, yMax}) => setNewData(d => ({...d, xMin, xMax, yMin, yMax})) }/>
            <div css={{
                display: 'flex',
                flexDirection: 'column',
                alignItems: 'center',
              ...
              <Color colorStr={newData.color}
                     onChange={ colorStr =>  setNewData(d => ({...d, color: colorStr})) }/>
              <span css={{paddingRight: '.5em',
                          paddingTop: '1em'}}>
                Mesh Size:
              </span>            
              <MeshSlider min={50} max={700} meshSize={newData.meshSize}
