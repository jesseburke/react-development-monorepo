...
import {CondtionalDisplay} from '@jesseburke/basic-react-components';
import {Button} from '@jesseburke/basic-react-components';

import FunctionBounds from './FunctionBounds.js';


export default function CameraOptions({ cameraData: {position},
                                        onChangeFunc }) {

    const [change, setChange] = useState(false);

    const comp = !change ? <DisplayCameraPositionComp position={position}
                                                      onClickFunc={() => setChange(true) }/>
    : <ChangeCameraComp startingPosition={position}
                        cbFunc={(pos) => {
                            setChange(false);
                            onChangeFunc(pos);
                        }}/>;
