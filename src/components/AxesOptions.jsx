    ...
    return (
        <div
            ...
            <JustAxes
                ...
                onChange={({ show, radius, color, showLabels }) =>
                    onChange({
                        ...axesData,
                        show: show,
                        radius: radius,
            ...
            <QuadSize
                size={length}
                onChange={(size) => onChange({ ...axesData, gridQuadSize: size })}
            />
        </div>
    );
}
...
function JustAxes({ show, radius, color, showLabels, onChange }) {
    ...
    return (
        <div
            ...
            <ConditionalDisplay test={show}>
                <Color
                    colorStr={color}
                    onChange={(colorStr) => onChange({ ...axesData, color: colorStr })}
                />
                <Radius
                    size={radius}
                    onChange={(size) => onChange({ ...axesData, radius: size })}
                />
                <div
                    css={{
                        display: 'flex',
                    ...
                    <label>
                        ...
                        <input
                            ...
                            checked={showLabels}
                            onChange={(event) =>
                                onChange({ ...axesData, showLabels: event.target.checked })
                            }
                        />
                    </label>
                </div>
...
function QuadSize({ size, onChange }) {
    return (
        <div
            ...
            <Input size={10} initValue={size} onC={(sizeStr) => onChange(parseInt(sizeStr))} />
        </div>
    );
}
