...
{    
	...
	if( !cameraData.orthographic ) {
	    ...
	    camera.current = new THREE.OrthographicCamera( cameraData.orthographic.left,
	...
	for( let key in threeLabelData.current ) {
	    ...
	    [x, y] = threeToHtmlCoords({ coord: threeLabelData.current[key].pos,
    ...
    function threeToHtmlCoords({coord, worldInverse, projection}) {
        ...
        const x = (temp.x *  .5 + .5) * width.current;
        const y = (temp.y * -.5 + .5) * height.current;
        return [x,y];
    }
	...
	// following accounts for fact that canvas might not be entire window
        const yperc = (e.clientY - canvasRef.current.offsetParent.offsetTop)/ canvasRef.current.clientHeight;
