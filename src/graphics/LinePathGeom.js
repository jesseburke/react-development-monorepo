...
const defaultRadius = .075;
const defaultVertexSize = 2;

export default function LinePathGeom( pointArr,
    ...
    for( let i = 0; i < l-1; i++ ) {
        path.add(               
            new THREE.LineCurve3( new THREE.Vector3( ...pointArr[i] ),
                                  new THREE.Vector3( ...pointArr[i+1] ) ));
	
	tempSphere = new THREE.SphereBufferGeometry( vertexSize*radius, 32, 32 )
	    .translate(...pointArr[i+1] );

	sphereArr.push(tempSphere);
    }

    const tubeGeometry = new THREE.TubeBufferGeometry( path, tubularSegments, radius, radialSegments, closed );
...
export function RegularNgonPts( n, size = 1 ) {
    ...
    for( let i = 0; i <= n; i++ ) {
	pointArr.push( [size*Math.cos( i*2*Math.PI/n + Math.PI/2),
			size*Math.sin( i*2*Math.PI/n+ Math.PI/2 ), 0] );
    }
...
export function RegularNgonSymmetrySlopes( n ) {
	...
	for( let i = 1; i <= n/2; i++ ) {
	    ...
	    const vec = new THREE.Vector3( pointArr[i-1][0], pointArr[i-1][1], pointArr[i-1][2] );
...
export function IrregularNgon( n,
			       randomFactor = .5,
			       radius = defaultRadius,
			       vertexSize = defaultVertexSize ) {
...
function IrregularNgonPts( n, randomFactor = .5 ) {
	...
	pointArr.push( [Math.cos( i*2*Math.PI/n + eps2) + eps1, Math.sin( i*2*Math.PI/n + eps1) + eps2, 0] );
    }
