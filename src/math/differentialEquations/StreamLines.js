import {RK4Pts} from './RK4.js';
import TriangulationFactory from '../../factories/TriangulationFactory.js';
import PriorityQueueFactory from '../../utils/PriorityQueueFactory.js';


// h is used to calculate solution curves

export default function Streamlines({ func, initialPt = [0,0], bounds, sepDist = .5, saturation = 1.5, h }) {

    const {xMin, xMax, yMin, yMax} = bounds;

    let pointArray = [], curveArray = [], curCurve;
    ...
    pointArray = pointsOnBoundary({ xMin: xMin - sepDist, xMax: xMax + sepDist,
				    yMin: yMin - sepDist, yMax: yMax + sepDist,
				    delta: sepDist });
	...
	const isStillTriangle = filterRadiusAndCenter(t.getRadiusAndCenterArray())
	      .some( ({center, radius}) =>
	...
	if( triangleArray.length > 1 ) {
	    triangleArray = triangleArray.filter( 
		({center, radius}, index) => {

		    if( index === 0 )
			return (radius >= triangleArray[1].radius);
	...
	filterRadiusAndCenter(t.halfedgesOfPoint( curCurve[curCurve.length-1] ) ).forEach( o => pq.push(o) );
    }
	...
	return (
	    array.filter( ({radius, center}) => radius > sepDist * saturation )
		.filter( ({radius, center}) =>
			 center[0] < xMax &&
			 center[0] > xMin &&
			 center[1] < yMax &&
			 center[1] > yMin )
